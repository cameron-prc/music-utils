class OauthController < ApplicationController
  ...
  def authorise_youtube
    session[:oauth_state] = SecureRandom.hex(16)
    redirect_to YoutubeClient::Authorisation.build_authorisation_url(state: session[:oauth_state]), allow_other_host: true
  end
  ...
  def callback_youtube
    ...
    tokens = YoutubeClient::Authorisation.request_access_tokens(params[:code])

    Current.user.oauth_tokens.find_or_initialize_by(provider: Providers::YOUTUBE).tap do |token|
      token.update!(
        access_token:  tokens[:access_token],
        refresh_token: tokens[:refresh_token],
        expires_at:    Time.now + tokens[:expires_in]
    ...
    redirect_to root_path, notice: "YouTube connected"
  end
end
